...
func CambioEstadoPago(estado_cumplido_id int, cumplido_proveedor_id int, documento_responsable string, cargo_responsable string) (respuesta_cambio_estado models.CambioEstadoCumplidoResponse, outputError map[string]interface{}) {
	defer func() {
	...
	if response, err := getJsonTest(beego.AppConfig.String("UrlCrudRevisionCumplidosProveedores")+"/cambio_estado_cumplido/?query=CumplidoProveedorId.Id:"+strconv.Itoa(cumplido_proveedor_id)+",Activo:true&sortby=FechaCreacion&order=desc", &respuesta_peticion); err == nil && response == 200 {
		LimpiezaRespuestaRefactor(respuesta_peticion, &cambios_anteriores)
		if (cambios_anteriores[0] != models.CambioEstadoCumplido{}) && cambios_anteriores[0].EstadoCumplidoId.Id == estado_cumplido_id && cambios_anteriores[0].Activo == true {
	...
	if response, err := getJsonTest(beego.AppConfig.String("UrlCrudRevisionCumplidosProveedores")+"/estado_cumplido/?query=Id:"+strconv.Itoa(estado_cumplido_id), &respuesta); (err == nil) && (response == 200) {
		LimpiezaRespuestaRefactor(respuesta, &estado_cumplido)
		if estado_cumplido != nil {
			var cumplido_proveedor []models.CumplidoProveedor
			if response, err := getJsonTest(beego.AppConfig.String("UrlCrudRevisionCumplidosProveedores")+"/cumplido_proveedor/?query=Id:"+strconv.Itoa(cumplido_proveedor_id), &respuesta); (err == nil) && (response == 200) {
